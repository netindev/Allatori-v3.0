/* ConstantHTML - Decompiled by JODE
...
final class ConstantHTML implements Constants
{
	...
	file.println(new StringBuilder().append("<H4> <A NAME=cp").append
			 (index).append
			 (">").append
			 (index).append
			 ("</A> ").append
			 (CONSTANT_NAMES[tag]).append
			 ("</H4>").toString());
	switch (tag) {
	case 10:
	case 11: {
		...
		= ((ConstantNameAndType)
		   constant_pool.getConstant(name_index, (byte) 12));
	    String signature
		...
		ref = new StringBuilder().append("<A HREF=\"").append
			  (class_name).append
			  ("_code.html#method").append
			  (getMethodNumber(new StringBuilder().append
					       (method_name).append
					       (signature).toString()))
			  .append
			  ("\" TARGET=Code>").append
			  (html_method_name).append
			  ("</A>").toString();
	    else
		ref = new StringBuilder().append("<A HREF=\"").append
			  (method_class).append
			  (".html").append
			  ("\" TARGET=_top>").append
			  (short_method_class).append
			  ("</A>.").append
			  (html_method_name).toString();
	    constant_ref[index] = new StringBuilder().append(ret_type).append
				      ("&nbsp;<A HREF=\"").append
				      (class_name).append
				      ("_cp.html#cp").append
				      (class_index).append
				      ("\" TARGET=Constants>").append
				      (short_method_class).append
				      ("</A>.<A HREF=\"").append
				      (class_name).append
				      ("_cp.html#cp").append
				      ...
				      ("</A>&nbsp;").append
				      (arg_types).toString();
	    file.println(new StringBuilder().append("<P><TT>").append
			     (ret_type).append
			     ("&nbsp;").append
			     (ref).append
			     (arg_types).append
	...
	case 9: {
		...
		ref = new StringBuilder().append("<A HREF=\"").append
			  ...
			  (short_field_class).append
			  ("</A>.").append
			  (field_name).append
			  ("\n").toString();
	    constant_ref[index]
		= new StringBuilder().append("<A HREF=\"").append
		      (class_name).append
		      ("_cp.html#cp").append
		      (class_index).append
		      ("\" TARGET=Constants>").append
		      ...
		      (field_name).append
		      ("</A>").toString();
	    file.println(new StringBuilder().append("<P><TT>").append(ref)
			     .append
			     ("</TT><BR>\n").append
			     ("<UL>").append
			     ("<LI><A HREF=\"#cp").append
	...
	case 7: {
	    ConstantClass c4
		= (ConstantClass) constant_pool.getConstant(index, (byte) 7);
	    ...
	    short_class_name
		= Utility.compactClassName(short_class_name,
					   new StringBuilder().append
					       (class_package).append
					       (".").toString(),
					   true);
	    String ref = new StringBuilder().append("<A HREF=\"").append
			     (class_name2).append
			     (".html\" TARGET=_top>").append
			     (short_class_name).append
			     ("</A>").toString();
	    constant_ref[index]
		= new StringBuilder().append("<A HREF=\"").append
		      (class_name).append
	...
	case 8: {
	    ConstantString c5
		= (ConstantString) constant_pool.getConstant(index, (byte) 8);
	    int name_index = c5.getStringIndex();
	    String str
		= Class2HTML.toHTML(constant_pool.constantToString(index,
								   tag));
	    file.println(new StringBuilder().append("<P><TT>").append(str)
			     .append
			     ("</TT><UL>").append
			     ("<LI><A HREF=\"#cp").append
			     (name_index).append
	...
	case 12: {
	    ...
	    file.println
		(new StringBuilder().append("<P><TT>").append
		     (Class2HTML.toHTML(constant_pool.constantToString(index,
								       tag)))
		     .append
		     ("</TT><UL>").append
	...
	for (int i = 0; i < methods.length; i++) {
	    String cmp
		= new StringBuilder().append(methods[i].getName()).append
		      (methods[i].getSignature()).toString();
	    if (cmp.equals(str))
		return i;
	}
	return -1;
