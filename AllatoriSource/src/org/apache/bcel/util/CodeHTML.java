...
final class CodeHTML implements Constants
{
	...
	file = new PrintWriter(new FileOutputStream(new StringBuilder().append
							(dir).append
							(class_name).append
							("_code.html")
							.toString()));
	...
	buf.append("<TT>").append(OPCODE_NAMES[opcode])
	    .append("</TT></TD><TD>");
	...
	case 170: {
		...
		buf.append("<TD><A HREF=\"#code").append(method_number).append
		    ...
		    (jump_table[i]).append
		    ("\">").append
		    (jump_table[i]).append("</A></TD>");
	    buf.append("<TD><A HREF=\"#code").append(method_number).append
		("@").append
		(default_offset).append
		("\">").append
		(default_offset).append("</A></TD></TR>\n</TABLE>\n");
	    break;
	}
	case 171: {
	...
	case 199: {
	    ...
	    buf.append("<A HREF=\"#code").append(method_number).append("@")
		.append
		(index).append
		("\">").append
		(index).append("</A>");
	...
	case 196:
	    ...
	    break;
	case 188:
	    buf.append("<FONT COLOR=\"#00FF00\">").append
		(TYPE_NAMES[bytes.readByte()]).append("</FONT>");
	    break;
	case 178:
	case 179:
	case 180:
	case 181: {
	    ...
	    ConstantFieldref c1 = ((ConstantFieldref)
				   constant_pool.getConstant(index, (byte) 9));
	    int class_index = c1.getClassIndex();
	    String name
		= constant_pool.getConstantString(class_index, (byte) 7);
	    name = Utility.compactClassName(name, false);
	    index = c1.getNameAndTypeIndex();
	    String field_name
		= constant_pool.constantToString(index, (byte) 12);
	    if (name.equals(class_name))
		buf.append("<A HREF=\"").append(class_name).append
		    ("_methods.html#field").append
		    (field_name).append
		    ("\" TARGET=Methods>").append
		    (field_name).append("</A>\n");
	    else
		buf.append(constant_html.referenceConstant(class_index)).append
		    (".").append(field_name);
	    break;
	}
	case 187:
	...
	case 185: {
		...
		ConstantInterfaceMethodref c
		    = ((ConstantInterfaceMethodref)
		       constant_pool.getConstant(m_index, (byte) 11));
		...
		ConstantMethodref c
		    = ((ConstantMethodref)
		       constant_pool.getConstant(m_index, (byte) 10));
		class_index = c.getClassIndex();
		String str = constant_pool.constantToString(c);
		index = c.getNameAndTypeIndex();
	    }
	    ...
	    String str
		= Class2HTML.toHTML(constant_pool.constantToString
				    (constant_pool.getConstant(index,
							       (byte) 12)));
	    ConstantNameAndType c2
		= ((ConstantNameAndType)
		   constant_pool.getConstant(index, (byte) 12));
	    String signature
	    ...
	    buf.append(name).append(".<A HREF=\"").append(class_name).append
		("_cp.html#cp").append
		(m_index).append
	...
	case 197: {
	    ...
	    buf.append(constant_html.referenceConstant(index)).append(":")
		.append
		(dimensions).append("-dimensional");
	    break;
	}
	case 132: {
	    int vindex;
    ...
    private final void findGotos
	(ByteSequence bytes, Method method, Code code) throws IOException {
	...
	file.print(new StringBuilder().append
		       ("<P><B><FONT COLOR=\"#FF0000\">").append
		       (access).append
		       ("</FONT>&nbsp;").append
		       ("<A NAME=method").append
		       (method_number).append
		       (">").append
		       (Class2HTML.referenceType(type)).append
		       ("</A>&nbsp<A HREF=\"").append
		       (class_name).append
		       ("_methods.html#method").append
		       (method_number).append
		       ("\" TARGET=Methods>").append
		       (html_name).append
		       ("</A>(").toString());
	...
	if (attributes.length > 0) {
		...
		if (tag != -1)
		    file.print(new StringBuilder().append("<LI><A HREF=\"")
				   .append
				   (class_name).append
				   ("_attributes.html#method").append
				   (method_number).append
				   ("@").append
				   (i).append
				   ("\" TARGET=Attributes>").append
				   (ATTRIBUTE_NAMES[tag]).append
				   ("</A></LI>\n").toString());
		else
		    file.print(new StringBuilder().append("<LI>").append
				   (attributes[i]).append
				   ("</LI>").toString());
		if (tag == 2) {
		    c = (Code) attributes[i];
	...
	if (code != null) {
		...
		if (goto_set.get(offset))
		    anchor = new StringBuilder().append("<A NAME=code").append
				 (method_number).append
				 ("@").append
				 (offset).append
				 ("></A>").toString();
		...
		if (stream.getIndex() == code.length)
		    anchor2 = new StringBuilder().append("<A NAME=code").append
				  (method_number).append
				  ("@").append
				  (code.length).append
				  (">").append
		    ...
		    anchor2 = new StringBuilder().append("").append(offset)
				  .toString();
		file.println(new StringBuilder().append
				 ("<TR VALIGN=TOP><TD>").append
				 (anchor2).append
				 ("</TD><TD>").append
				 (anchor).append
				 (str).append
