...
public class BCELifier extends EmptyVisitor
{
	...
	String inter
	    = Utility.printArray(clazz.getInterfaceNames(), false, true);
	if (!"".equals(package_name)) {
	    class_name = class_name.substring(package_name.length() + 1);
	    _out.println(new StringBuilder().append("package ").append
			     (package_name).append
			     (";").toString());
	    _out.println();
	}
	...
	_out.println();
	_out.println(new StringBuilder().append("public class ").append
			 (class_name).append
			 ("Creator implements Constants {").toString());
	...
	_out.println();
	_out.println(new StringBuilder().append("  public ").append
			 (class_name).append
			 ("Creator() {").toString());
	_out.println(new StringBuilder().append
			 ("    _cg = new ClassGen(\"").append
			 ("".equals(package_name) ? class_name
			  : new StringBuilder().append(package_name).append
				(".").append
				(class_name).toString())
			 .append
			 ("\", \"").append
			 ...
			 ("\", ").append
			 ("\"").append
			 (clazz.getSourceFileName()).append
			 ("\", ").append
			 (printFlags(clazz.getAccessFlags(), 0)).append
			 (", ").append
			 ("new String[] { ").append
			 (inter).append
			 (" });").toString());
	...
	for (int i = 0; i < methods.length; i++) {
	    _out.println(new StringBuilder().append
			     ("  private void createMethod_").append
			     (i).append
			     ("() {").toString());
	...
	for (int i = 0; i < methods.length; i++)
	    _out.println(new StringBuilder().append("    createMethod_").append
			     (i).append
			     ("();").toString());
	...
	_out.println(new StringBuilder().append("    ").append(class_name)
			 .append
			 ("Creator creator = new ").append
			 (class_name).append
			 ("Creator();").toString());
	_out.println(new StringBuilder().append
			 ("    creator.create(new FileOutputStream(\"").append
			 (class_name).append
			 (".class\"));").toString());
	_out.println("  }");
    }
	...
	_out.println();
	_out.println(new StringBuilder().append
			 ("    field = new FieldGen(").append
			 (printFlags(field.getAccessFlags())).append
			 (", ").append
			 (printType(field.getSignature())).append
			 (", \"").append
	...
	_out.println(new StringBuilder().append
			 ("    MethodGen method = new MethodGen(").append
			 (printFlags(method.getAccessFlags(), 1)).append
			 (", ").append
			 (printType(result_type)).append
			 (", ").append
			 (printArgumentTypes(arg_types)).append
			 (", ").append
			 ("new String[] { ").append
			 (Utility.printArray(mg.getArgumentNames(), false,
					     true))
			 .append
			 (" }, \"").append
			 (method.getName()).append
			 ("\", \"").append
			 (_clazz.getClassName()).append
			 ("\", il, _cp);").toString());
	_out.println();
	...
	while (pow <= 16384) {
		...
		if (pow == 32 && reason == 0)
		    ...
		    buf.append("ACC_").append
			(Constants.ACCESS_NAMES[i].toUpperCase(Locale.ENGLISH))
			.append(" | ");
	    }
	    pow <<= 1;
	    i++;
	...
	return new StringBuilder().append("new Type[] { ").append
		   (args.toString()).append
		   (" }").toString();
    }
	...
	if (type instanceof ArrayType) {
	    ...
	    return new StringBuilder().append("new ArrayType(").append
		       (printType(at.getBasicType())).append
		       (", ").append
		       (at.getDimensions()).append
		       (")").toString();
	}
	return new StringBuilder().append("new ObjectType(\"").append
		   (Utility.signatureToString(signature, false)).append
		   ("\")").toString();
    }
	...
	if ((java_class = org.apache.bcel.Repository.lookupClass(name))
	    == null)
