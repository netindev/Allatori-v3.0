...
public class Class2HTML implements Constants {
		...
		try {
			for (int i = 0; i < args.length; i++) {
				if (args[i].charAt(0) == '-') {
					if (args[i].equals("-d")) {
						dir = args[++i];
						if (!dir.endsWith(new StringBuilder().append("").append(sep).toString()))
							dir = new StringBuilder().append(dir).append(sep).toString();
						new File(dir).mkdirs();
					} else if (args[i].equals("-zip"))
						zip_file = args[++i];
					else
						System.out.println(new StringBuilder().append("Unknown option ").append(args[i]).toString());
				} else
					file_name[files++] = args[i];
			}
			if (files == 0)
				...
				for (int i = 0; i < files; i++) {
					System.out.print(
							new StringBuilder().append("Processing ").append(file_name[i]).append("...").toString());
					if (zip_file == null)
	...
	static String referenceClass(int index) {
		...
		str = Utility.compactClassName(str, new StringBuilder().append(class_package).append(".").toString(), true);
		return new StringBuilder().append("<A HREF=\"").append(class_name).append("_cp.html#cp").append(index)
				.append("\" TARGET=ConstantPool>").append(str).append("</A>").toString();
	}
	...
	static final String referenceType(String type) {
		...
		if (base_type.equals("int") || base_type.equals("short") || base_type.equals("boolean")
				...
				|| base_type.equals("long") || base_type.equals("double") || base_type.equals("float"))
			return new StringBuilder().append("<FONT COLOR=\"#00FF00\">").append(type).append("</FONT>").toString();
		return new StringBuilder().append("<A HREF=\"").append(base_type).append(".html\" TARGET=_top>")
				.append(short_type).append("</A>").toString();
	}
	...
	private void writeMainHTML(AttributeHTML attribute_html) throws IOException {
		...
		file.println(new StringBuilder().append("<HTML>\n<HEAD><TITLE>Documentation for ").append(class_name)
				...
				.append(class_name).append("_cp.html").append("\"\n MARGINWIDTH=\"0\" ")
				.append("MARGINHEIGHT=\"0\" FRAMEBORDER=\"1\" SCROLLING=\"AUTO\">\n")
				.append("<FRAME NAME=\"Attributes\" SRC=\"").append(class_name).append("_attributes.html")
				...
				.append("MARGINHEIGHT=0 FRAMEBORDER=1 SCROLLING=\"AUTO\">\n").append("<FRAME NAME=\"Methods\" SRC=\"")
				.append(class_name).append("_methods.html\"\n MARGINWIDTH=0 ")
				.append("MARGINHEIGHT=0 FRAMEBORDER=1 SCROLLING=\"AUTO\">\n").append("</FRAMESET></FRAMESET></HTML>")
				.toString());
		file.close();
		for (int i = 0; i < attributes.length; i++)
			attribute_html.writeAttribute(attributes[i], new StringBuilder().append("class").append(i).toString());
	}
}
