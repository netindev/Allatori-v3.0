...
final class AttributeHTML implements Constants
{
	...
	if (tag != -1) {
	    ...
	    file.println(new StringBuilder().append("<H4><A NAME=\"").append
			     (anchor).append
			     ("\">").append
			     (attr_count).append
			     (" ").append
		...
		file.print(new StringBuilder().append
			       ("<UL><LI>Maximum stack size = ").append
			       (c.getMaxStack()).append
			       ("</LI>\n<LI>Number of local variables = ")
			       .append
			       (c.getMaxLocals()).append
			       ("</LI>\n<LI><A HREF=\"").append
			       (class_name).append
		...
		if (len > 0) {
			...
			file.print("<LI>");
			if (catch_type != 0)
			    file.print(constant_html
					   .referenceConstant(catch_type));
			else
			    file.print("Any Exception");
			file.print(new StringBuilder().append
				       ("<BR>(Ranging from lines ").append
				       (codeLink(ce[i].getStartPC(),
						 method_number))
				       .append
				       (" to ").append
				       (codeLink(ce[i].getEndPC(),
						 method_number))
				       .append
				       (", handled at line ").append
	    ...
	    case 1: {
		int index
		    = ((ConstantValue) attribute).getConstantValueIndex();
		file.print
		    (new StringBuilder().append("<UL><LI><A HREF=\"").append
			 (class_name).append
			 ("_cp.html#cp").append
			 (index).append
		...
		for (int i = 0; i < indices.length; i++)
		    file.print
			(new StringBuilder().append("<LI><A HREF=\"").append
			     (class_name).append
			     ("_cp.html#cp").append
			     (indices[i]).append
		...
		for (int i = 0; i < line_numbers.length; i++) {
		    file.print(new StringBuilder().append("(").append
				   (line_numbers[i].getStartPC()).append
				   (",&nbsp;").append
				   (line_numbers[i].getLineNumber()).append
				   (")").toString());
		...
		for (int i = 0; i < vars.length; i++) {
		    ...
		    String signature
			= ((ConstantUtf8)
			   constant_pool.getConstant(index, (byte) 1))
			      .getBytes();
		    signature = Utility.signatureToString(signature, false);
		    int start = vars[i].getStartPC();
		    int end = start + vars[i].getLength();
		    file.println(new StringBuilder().append("<LI>").append
				     (Class2HTML.referenceType(signature))
				     .append
				     ("&nbsp;<B>").append
				     (vars[i].getName()).append
				     ("</B> in slot %").append
				     (vars[i].getIndex()).append
				     ("<BR>Valid from lines ").append
				     ("<A HREF=\"").append
				     (class_name).append
				     ("_code.html#code").append
				     (method_number).append
	    ...
	    case 6: {
		InnerClass[] classes
		    = ((InnerClasses) attribute).getInnerClasses();
		file.print("<UL>");
		for (int i = 0; i < classes.length; i++) {
			...
			name = "&lt;anonymous&gt;";
		    String access
			= Utility.accessToString(classes[i]
						     .getInnerAccessFlags());
		    file.print(new StringBuilder().append
				   ("<LI><FONT COLOR=\"#FF0000\">").append
				   (access).append
				   ("</FONT> ").append
	    ...
	    default:
		file.print(new StringBuilder().append("<P>").append
