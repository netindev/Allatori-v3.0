...
public class AnnotationEntryGen
{
	...
	AnnotationEntry a
	    = new AnnotationEntry(typeIndex, cpool.getConstantPool(),
				  isRuntimeVisible);
    ...
    public AnnotationEntryGen(ObjectType type, List elements, boolean vis,
	...
	(DataInputStream dis, ConstantPoolGen cpool, boolean b)
	throws IOException {
	...
	for (int i = 0; i < elemValuePairCount; i++) {
		...
		(new ElementValuePairGen(nidx,
					 ElementValueGen
					     .readElementValue(dis, cpool),
					 cpool));
	}
	...
	s.append(new StringBuilder().append("AnnotationGen:[").append
		     (getTypeName()).append
		     (" #").append
		     (evs.size()).append
		     (" {").toString());
	...
	s.append(new StringBuilder().append("@").append(getTypeName()).append
		     ("(").toString());
	for (int i = 0; i < evs.size(); i++) {
	    s.append(evs.get(i));
	    if (i + 1 < evs.size())
