...
public class ClassGen extends AccessFlags implements Cloneable
{
	...
	for (int i = 0; i < attrs.length; i++) {
		...
		for (int j = 0; j < annos.length; j++) {
		    ...
		    annotationGenObjs.add
			(new AnnotationEntryGen(a, getConstantPool(), false));
		}
	...
	return ((AnnotationEntryGen[])
		annotationGenObjs.toArray(new AnnotationEntryGen
	...
	if (annotation_vec.isEmpty())
	    ...
	    attributes
		= new Attribute[attribute_vec.size() + annAttributes.length];
	    attribute_vec.toArray(attributes);
	    System.arraycopy(annAttributes, 0, attributes,
			     attribute_vec.size(), annAttributes.length);
	}
	ConstantPool _cp = cp.getFinalConstantPool();
	return new JavaClass(class_name_index, superclass_name_index,
			     file_name, major, minor, access_flags, _cp,
	...
	return ((Attribute[])
		attribute_vec.toArray(new Attribute[attribute_vec.size()]));
    }
    
    public AnnotationEntryGen[] getAnnotationEntries() {
	return ((AnnotationEntryGen[])
		annotation_vec
		    .toArray(new AnnotationEntryGen[annotation_vec.size()]));
    }
	...
	class_name = cp.getConstantPool().getConstantString
			 (class_name_index, (byte) 7).replace('/', '.');
    }
    
    public void setSuperclassNameIndex(int superclass_name_index) {
	this.superclass_name_index = superclass_name_index;
	super_class_name
	    = cp.getConstantPool().getConstantString
		  (superclass_name_index, (byte) 7).replace('/', '.');
    }
