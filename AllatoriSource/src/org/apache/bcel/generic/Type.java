...
public abstract class Type implements Serializable
{
    ...
    public static final Type UNKNOWN = new Type((byte) 15,
						"<unknown object>") {
	...
	return (equals(NULL) || type >= 15 ? signature
	...
	if (cl.isPrimitive()) {
	    ...
	    throw new IllegalStateException
		      (new StringBuilder().append
			   ("Ooops, what primitive type is ").append
			   (cl).toString());
	}
    ...
    static final int getTypeSize(String signature)
	throws StringIndexOutOfBoundsException {
	byte type = Utility.typeOfSignature(signature);
	if (type <= 12)
