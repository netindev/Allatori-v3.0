    ...
    implements Cloneable, Constants, Node, Serializable
{
	...
	return new StringBuilder().append("CodeException(start_pc = ").append
		   (start_pc).append
		   (", end_pc = ").append
		   (end_pc).append
		   (", handler_pc = ").append
	...
	if (catch_type == 0)
	    ...
	    str = new StringBuilder().append
		      (Utility.compactClassName
		       (cp.getConstantString(catch_type, (byte) 7), false))
		      .append
		      (verbose ? new StringBuilder().append("(").append
				     (catch_type).append
				     (")").toString() : "")
		      .toString();
	return new StringBuilder().append(start_pc).append("\t").append
		   (end_pc).append
		   ("\t").append
		   (handler_pc).append
		   ("\t").append
