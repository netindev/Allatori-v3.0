/* StackMapTableEntry - Decompiled by JODE
...
public final class StackMapTableEntry implements Cloneable, Serializable
{
	...
	if (frame_type >= 0 && frame_type <= 63)
		...
		types_of_stack_items[i]
		    = new StackMapType(file, constant_pool);
	} else
    ...
    public StackMapTableEntry
	(int tag, int byte_code_offset_delta, int number_of_locals,
	...
	if (frame_type < 0 || frame_type > 63) {
		...
		throw new ClassFormatException
			  (new StringBuilder().append
			       ("Invalid Stack map table tag: ").append
			       (frame_type).toString());
	}
    }
	...
	if (frame_type >= 0 && frame_type <= 63)
	    ...
	    buf.append(new StringBuilder().append("CHOP ").append
			   (251 - frame_type).toString());
	else if (frame_type == 251)
	    buf.append("SAME_EXTENDED");
	else if (frame_type >= 252 && frame_type <= 254)
	    buf.append(new StringBuilder().append("APPEND ").append
			   (frame_type - 251).toString());
	else if (frame_type == 255)
	    buf.append("FULL");
	else
	    buf.append("UNKNOWN");
