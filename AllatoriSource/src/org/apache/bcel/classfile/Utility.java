...
public abstract class Utility
{
	...
	try {
		...
		if (length < 0 || i < length) {
		    String indices = fillup(new StringBuilder().append
						(stream.getIndex()).append
						(":").toString(),
					    6, true, ' ');
		    buf.append(indices).append
			(codeToString(stream, constant_pool, verbose))
			.append('\n');
		}
		i++;
    ...
    public static final String codeToString
	(ByteSequence bytes, ConstantPool constant_pool, boolean verbose)
	throws IOException {
	...
	case 171: {
		...
		jump_table[i] = offset + bytes.readInt();
		buf.append("(").append(match[i]).append(", ").append
		    (jump_table[i]).append(")");
		if (i < npairs - 1)
		    buf.append(", ");
	    }
	...
	case 199:
	    buf.append("\t\t#")
		.append(bytes.getIndex() - 1 + bytes.readShort());
	    break;
	case 200:
	case 201:
	...
	case 188:
	    buf.append("\t\t<").append
		(Constants.TYPE_NAMES[bytes.readByte()]).append(">");
	    break;
	case 178:
	case 179:
	case 180:
	case 181: {
	    ...
	    buf.append("\t\t").append
		(constant_pool.constantToString(index, (byte) 9)).append
		(verbose ? new StringBuilder().append(" (").append(index)
			       .append
			       (")").toString() : "");
	    break;
	}
	...
	case 193: {
	    int index = bytes.readUnsignedShort();
	    buf.append("\t<").append
		(constant_pool.constantToString(index, (byte) 7)).append
		(">").append
		(verbose ? new StringBuilder().append(" (").append(index)
			       .append
			       (")").toString() : "");
	...
	case 184: {
	    int index = bytes.readUnsignedShort();
	    buf.append("\t").append
		(constant_pool.constantToString(index, (byte) 10)).append
		(verbose ? new StringBuilder().append(" (").append(index)
			       .append
			       (")").toString() : "");
	    break;
	...
	case 185: {
	    ...
	    buf.append("\t").append
		(constant_pool.constantToString(index, (byte) 11)).append
		(verbose ? new StringBuilder().append(" (").append(index)
			       .append
			       (")\t").toString() : "")
		.append
	...
	default:
		...
		for (int i = 0; i < Constants.TYPE_OF_OPERANDS[opcode].length;
		     i++) {
		    buf.append("\t\t");
		    ...
		    default:
			System.err
			    .println("Unreachable default case reached!");
			System.exit(-1);
		    }
		}
	    }
    ...
    public static final String codeToString
	(ByteSequence bytes, ConstantPool constant_pool) throws IOException {
    ...
    public static final String methodTypeToSignature
	(String ret, String[] argv) throws ClassFormatException {
	StringBuilder buf = new StringBuilder("(");
	if (argv != null) {
    ...
    public static final String[] methodSignatureArgumentTypes
	(String signature) throws ClassFormatException {
	return methodSignatureArgumentTypes(signature, true);
    }
    
    public static final String[] methodSignatureArgumentTypes
	(String signature, boolean chopit) throws ClassFormatException {
    ...
    public static final String methodSignatureReturnType
	(String signature, boolean chopit) throws ClassFormatException {
	String type;
	try {
	...
	try {
		...
		throw new ClassFormatException
			  (new StringBuilder().append
			       ...
			       (signature).toString());
	    int index;
	    for (index = 1; signature.charAt(index) != ')';
		 index += unwrap(consumed_chars)) {
		String param_type
	...
	buf.append(")");
	return new StringBuilder().append(access).append
		   (access.length() > 0 ? " " : "").append
		   (type).append
		   (" ").append
		   (name).append
	...
	try {
		...
		int old_index;
		for (old_index = 0;
		     (index = str.indexOf(old, old_index)) != -1;
	...
	for (int i = 0; i < chars.length; i++) {
		...
		if (open)
		    throw new RuntimeException
			      (new StringBuilder().append
				   ("Illegally nested brackets:").append
				   (brackets).toString());
		open = true;
		break;
	    case ']':
	...
	for (int j = 0; j < buf.length; j++)
	    buf[j] = fill;
	if (left_justify)
	    return new StringBuilder().append(str).append(new String(buf))
		       .toString();
	return new StringBuilder().append(new String(buf)).append(str)
		   .toString();
    }
    ...
    public static Attribute[] getAnnotationAttributes(ConstantPoolGen cp,
						      List vec) {
	if (vec.isEmpty())
	...
	try {
	    ...
	    System.err.println
		("IOException whilst processing parameter annotations");
	    e.printStackTrace();
	    return null;
	}
	return attributes;
