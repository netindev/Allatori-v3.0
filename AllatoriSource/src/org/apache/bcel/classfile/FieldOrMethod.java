    ...
    implements Cloneable, Node
{
	...
	ConstantUtf8 c
	    = (ConstantUtf8) constant_pool.getConstant(name_index, (byte) 1);
	return c.getBytes();
    }
	...
	if (annotationsOutOfDate) {
		...
		if (attribute instanceof Annotations) {
		    ...
		    for (int j = 0;
			 j < annotations.getAnnotationEntries().length; j++)
			accumulatedAnnotations
			    .add(annotations.getAnnotationEntries()[j]);
		}
	    }
	    annotationEntries
		= ((AnnotationEntry[])
		   accumulatedAnnotations.toArray(new AnnotationEntry
						  [accumulatedAnnotations
						       .size()]));
	...
	if (!searchedForSignatureAttribute) {
		...
		if (attributes[i] instanceof Signature) {
			...
			= ((Signature) attributes[i]).getSignature();
		    found = true;
		}
	    }
