...
public final class InnerClass implements Cloneable, Node, Serializable
{
	...
	return new StringBuilder().append("InnerClass(").append
		   (inner_class_index).append
		   (", ").append
		   (outer_class_index).append
		   (", ").append
		   (inner_name_index).append
		   (", ").append
		   (inner_access_flags).append
		   (")").toString();
    }
    
    public final String toString(ConstantPool constant_pool) {
	String inner_class_name
	    = constant_pool.getConstantString(inner_class_index, (byte) 7);
	inner_class_name = Utility.compactClassName(inner_class_name);
	String outer_class_name;
	if (outer_class_index != 0) {
	    outer_class_name
		= constant_pool.getConstantString(outer_class_index, (byte) 7);
	...
	if (inner_name_index != 0)
	    inner_name
		= ((ConstantUtf8)
		   constant_pool.getConstant(inner_name_index, (byte) 1))
		      .getBytes();
	else
	...
	return new StringBuilder().append("InnerClass:").append(access).append
		   (inner_class_name).append
		   ("(\"").append
		   (outer_class_name).append
		   ("\", \"").append
		   (inner_name).append
		   ("\")").toString();
    }
