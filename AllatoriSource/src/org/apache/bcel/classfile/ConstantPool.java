...
public class ConstantPool implements Cloneable, Node, Serializable {
	...
	public String constantToString(Constant c) throws ClassFormatException {
		...
		case 3:
			...
			break;
		case 12:
			str = new StringBuilder().append(constantToString(((ConstantNameAndType) c).getNameIndex(), (byte) 1))
					.append(" ").append(constantToString(((ConstantNameAndType) c).getSignatureIndex(), (byte) 1))
					.toString();
			break;
		case 9:
		case 10:
		case 11:
			str = new StringBuilder().append(constantToString(((ConstantCP) c).getClassIndex(), (byte) 7)).append(".")
					.append(constantToString(((ConstantCP) c).getNameAndTypeIndex(), (byte) 12)).toString();
			break;
		default:
			throw new RuntimeException(new StringBuilder().append("Unknown constant type ").append(tag).toString());
		}
		return str;
	}
	...
	public Constant getConstant(int index) {
		if (index >= constant_pool.length || index < 0)
			throw new ClassFormatException(new StringBuilder().append("Invalid constant pool reference: ").append(index)
					.append(". Constant pool size is: ").append(constant_pool.length).toString());
	...
	public Constant getConstant(int index, byte tag) throws ClassFormatException {
		...
		if (c.getTag() != tag)
			throw new ClassFormatException(
					new StringBuilder().append("Expected class `").append(Constants.CONSTANT_NAMES[tag])
							.append("' at index ").append(index).append(" and got ").append(c).toString());
		return c;
	}
	...
	public String getConstantString(int index, byte tag) throws ClassFormatException {
		...
		default:
			throw new RuntimeException(
					new StringBuilder().append("getConstantString called with illegal tag ").append(tag).toString());
		}
		c = getConstant(i, (byte) 1);
