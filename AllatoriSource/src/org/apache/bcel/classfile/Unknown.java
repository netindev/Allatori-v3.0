...
public final class Unknown extends Attribute
{
	...
	this.bytes = bytes;
	name = ((ConstantUtf8) constant_pool.getConstant(name_index, (byte) 1))
		   .getBytes();
	...
	if (length == 0 || bytes == null)
	    return new StringBuilder().append("(Unknown attribute ").append
		       (name).append
		       (")").toString();
	String hex;
	if (length > 10) {
	    ...
	    hex = new StringBuilder().append(Utility.toHexString(tmp)).append
		      ("... (truncated)").toString();
	} else
