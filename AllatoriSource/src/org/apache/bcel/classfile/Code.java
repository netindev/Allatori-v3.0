...
public final class Code extends Attribute
{
	...
	this(c.getNameIndex(), c.getLength(), c.getMaxStack(),
	     c.getMaxLocals(), c.getCode(), c.getExceptionTable(),
	...
	this(name_index, length, file.readUnsignedShort(),
	     file.readUnsignedShort(), null, null, null, constant_pool);
	...
	exception_table_length
	    = exception_table == null ? 0 : exception_table.length;
	length = calculateLength();
    }
	...
	buf.append("Code(max_stack = ").append(max_stack).append
	    (", max_locals = ").append
	    (max_locals).append
	    (", code_length = ").append
	    (code_length).append
	...
	if (exception_table_length > 0) {
	    ...
	    for (int i = 0; i < exception_table_length; i++)
		buf.append
		    (exception_table[i].toString(constant_pool, verbose))
		    .append("\n");
	}
